...
| 3. Expediente documental | En implementacion | Soportar PDFs y su vinculo con vehiculo y verificacion. | `documents`, `document_files` | Validar la capa S3-compatible en bucket real, ejecutar el cutover de archivos previos a `OBJECT_STORAGE` y cerrar trazabilidad operativa del expediente. | Fases 1 y 2. | Subida, lectura, migracion y consulta de PDFs vigentes. | Poder consultar tarjeta y constancias desde el expediente. |
...
2. validar la **fase 3** con `OBJECT_STORAGE` S3-compatible y migracion operativa de PDFs antes de abrir notificaciones;
